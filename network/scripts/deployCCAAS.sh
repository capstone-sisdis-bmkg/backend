...
startDockerContainer() {
  ...
  if [ "$CCAAS_DOCKER_RUN" = "true" ]; then
    ...
    set -x
    ${CONTAINER_CLI} run --rm -d --name peer0bmkg_${CC_NAME}_ccaas  \
                  --network bmkg_network \
    ...
    infoln "    ${CONTAINER_CLI} run --rm -d --name peer0bmkg_${CC_NAME}_ccaas  \
                  --network bmkg_network \
                  -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:${CCAAS_SERVER_PORT} \
                  -e CHAINCODE_ID=$PACKAGE_ID -e CORE_CHAINCODE_ID_NAME=$PACKAGE_ID \
                    ${CC_NAME}_ccaas_image:latest"
...
## Install chaincode on peer0.bmkg and peer0.supplychain
infoln "Installing chaincode on peer0.bmkg..."
installChaincode 1
...
## approve the definition for bmkg
approveForMyOrg 1

## check whether the chaincode definition is ready to be committed
## expect bmkg to have approved and supplychain not to
checkCommitReadiness 1 "\"BMKGMSP\": true"
# checkCommitReadiness 2 "\"BMKGMSP\": true" "\"SupplyChainMSP\": false"
...
# checkCommitReadiness 1 "\"BMKGMSP\": true" "\"SupplyChainMSP\": true"
# checkCommitReadiness 2 "\"BMKGMSP\": true" "\"SupplyChainMSP\": true"
