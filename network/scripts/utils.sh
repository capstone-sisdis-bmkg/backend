...
function printHelp() {
  ...
  if [ "$USAGE" == "prereq" ]; then
    ...
    println "    -c <channel name> - Name of channel to create (defaults to \"bmkgchannel\")"
    ...
    println
    println " Examples:"
    println "   network.sh up createChannel -ca -c bmkgchannel -s couchdb "
    println "   network.sh up createChannel -bft -c bmkgchannel -s couchdb "
    ...
    println "    -ccep <policy>  - (Optional) Chaincode endorsement policy using signature policy syntax. The default policy requires an endorsement from BMKG and SupplyChain"
