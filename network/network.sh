...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    while :
    do
      if [ ! -f "organizations/fabric-ca/bmkg/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
    ...
    infoln "Creating BMKG Identities"
  ...
  infoln "Generating CCP files for BMKG"
...
# call the script to create the channel, join the peers of bmkg and supplychain,
...
function networkDown() {
  ...
  # stop org3 containers also in addition to bmkg and supplychain, in case we were running sample to add org3
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    ${CONTAINER_CLI} volume rm docker_orderer.example.com docker_peer0.bmkg.example.com docker_peer0.supplychain.example.com
    ...
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/bmkg/msp organizations/fabric-ca/bmkg/tls-cert.pem organizations/fabric-ca/bmkg/ca-cert.pem organizations/fabric-ca/bmkg/IssuerPublicKey organizations/fabric-ca/bmkg/IssuerRevocationPublicKey organizations/fabric-ca/bmkg/fabric-ca-server.db'
